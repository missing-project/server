...
class UserService {
  ...
  async loginUser(loginInfo:LoginInterface){
    ...
    // 계정 가입 내역 확인
    if(!user){
      throw new Error('입력하신 ID의 가입 내역이 없습니다. 다시 한 번 입력해 주세요');
    }
    ...
    const refreshToken = jwt.sign(refreshPayload,secretKey,{expiresIn:'14d'});
    
    //DB에 refresh token 저장
    
    const loginUser = await this.User.findOneAndUpdate({uid : user.uid},{refreshToken:refreshToken},{returnOriginal:false});
  ...
  async expandToken(uid: string, refreshToken: string){
   ...
   if(refreshToken === user?.refreshToken){
    ...
    const accessPayload = {uid: user.uid, role:user.role, device: user.role }
